...
export class UpdateUserCommandHandler implements RequestHandler<UpdateUserCommand,ApiDataResponse<UpdateUserResponseDto>>{
  ...
  public async handle(value: UpdateUserCommand): Promise<ApiDataResponse<UpdateUserResponseDto>> {
    ...
    try
    {
      ...
      await this.pipeline.step(UpdateUserSteps.DbService,async ()=>{
        ...
        const mapEntityUserResult=this.pipeline.getResult<UserEntity>(UpdateUserSteps.MapEntityService);
      ...
      const response=await this.pipeline.step(UpdateUserSteps.MapResponseService,async ()=>{
        const userEntityResult=this.pipeline.getResult<UserEntity>(UpdateUserSteps.DbService);

        const result=await this._updateUserMapResponseService.handleAsync(userEntityResult);
        return result;
      });
      ...
      // return
      return DataResponseFactory.success(StatusCodes.OK,response,`user updated successfully`);

    }
    catch(ex){
      return await DataResponseFactory.pipelineError(ex as Error | PipelineWorkflowException, queryRunner);
    }
    finally{
