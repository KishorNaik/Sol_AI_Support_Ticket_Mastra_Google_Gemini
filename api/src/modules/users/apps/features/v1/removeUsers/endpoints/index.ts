...
class RemoveUserCommandHandler
	implements RequestHandler<RemoveUserCommand, ApiDataResponse<RemoveUserResponseDto>>
{
		...
		try {
			...
			await this.pipeline.step(RemoveUserSteps.DbService, async () => {
				...
				const mapEntityUserResult = this.pipeline.getResult<UserEntity>(
					RemoveUserSteps.MapEntityService
				);
			...
			// Return
			const response: RemoveUserResponseDto = this.pipeline.getResult(
				RemoveUserSteps.MapResponseService
			);
