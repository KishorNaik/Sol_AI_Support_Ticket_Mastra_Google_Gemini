...
Container.set<AddProgrammingLanguageDbService>(
	AddProgrammingLanguageDbService,
	new AddProgrammingLanguageDbService()
);
...
export interface ICreateProgrammingLanguageDbService
	extends IServiceHandlerAsync<
		ICreateProgrammingLanguageDbServiceParameters,
		ProgrammingLanguageEntity
	> {}
...
export class CreateProgrammingLanguageDbService implements ICreateProgrammingLanguageDbService {
	...
	public handleAsync(
		params: ICreateProgrammingLanguageDbServiceParameters
	): Promise<Result<ProgrammingLanguageEntity, ResultError>> {
		return tryCatchResultAsync(async () => {
			...
			// Db
			const result = await this._addProgrammingLanguageDbService.handleAsync(
				entity,
				queryRunner
			);
			if (result.isErr()) {
